    ...
    Scaffold(
        ...
        group?.let { groupEntity ->
            Column(modifier = Modifier.padding(paddingValues).padding(16.dp)) {
                Text(text = groupEntity.groupName, style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold)
                ...
                Text(text = "${completed} of ${tasks.size} tasks completed", style = MaterialTheme.typography.bodySmall)
                Spacer(modifier = Modifier.height(8.dp))
                // Members list
                Text(text = "Members", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                members.forEach { member ->
                    Text(text = "• ${member.memberName} (${member.role})", style = MaterialTheme.typography.bodyMedium)
                }
                Spacer(modifier = Modifier.height(16.dp))
                // Tasks list
                Text(text = "Tasks", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
...
@Composable
private fun TaskItem(task: TaskEntity, members: List<MemberEntity>, onStatusChange: (TaskStatus) -> Unit) {
    Card(
        ...
        Column(modifier = Modifier.padding(8.dp)) {
            ...
            assignee?.let { Text(text = "Assigned to: ${it.memberName}", style = MaterialTheme.typography.bodySmall) }
            if (task.deadline != null) {
                val dateString = java.time.Instant.ofEpochMilli(task.deadline).atZone(java.time.ZoneId.systemDefault()).toLocalDate().toString()
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly) {
                TaskStatus.values().forEach { status ->
                    ...
                    FilterChip(
                        ...
                        label = { Text(text = status.name.replaceFirstChar { it.uppercaseChar() }) }
                    )
                }
            }
        }
